<!-- Vendored from hmcts/cpp-claude@61f849f (.claude/context/logging-standards.md) on 2026-08-20.
...
>
> - The correlation set `requestId` / `hearingId` / `hearingDay` **is permitted at `INFO`**, and every
>   processing log line MUST carry `requestId` and `hearingId`. Defendant PII at `INFO` or above
>   remains forbidden, at any level in a deployed environment for whole payloads.
> - There is **no REST API**, so there are no request/response or `@ControllerAdvice` log points to
>   standardise — the units of work are queue messages and per-authority submissions.
> - JSON logging is configured unconditionally in `src/main/resources/logback.xml`; there is no
>   profile-switched plain-text alternative to fall back on.
