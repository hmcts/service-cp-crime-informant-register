<!-- Vendored from hmcts/cpp-claude@61f849f (.claude/context/azure-sdk-guide.md) on 2026-08-20.
...
>
> - Messaging uses the **raw `com.azure:azure-messaging-servicebus` `ServiceBusProcessorClient`**
>   with peek-lock and explicit settlement — not the Spring Cloud Azure Service Bus starter, whose
>   auto-settlement model is incompatible with the settlement gate.
> - Secrets and connection strings are mounted by the **Key Vault Secrets Store CSI driver** under
>   workload identity; the service does **not** build an application-level Key Vault client.
> - There is **no REST API** here, so guidance about REST clients, controllers or Pact contracts is
>   background only.
> - The permitted `INFO` correlation set is `requestId` / `hearingId` / `hearingDay`; defendant PII
>   at `INFO` or above remains forbidden.
