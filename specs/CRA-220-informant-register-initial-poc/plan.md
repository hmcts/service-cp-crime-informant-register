...
| `spring.datasource.hikari.data-source-properties.socketTimeout` | `30` (seconds) | A store that dies without closing connections must not block the health probe indefinitely — without this, intake would never suspend on a silent store death (implementation finding, Batch E) |
...
| VII | Privacy in Telemetry | **PASS** — correlation set only via MDC: what the request names (`requestId`, `hearingId`, `hearingDay`, `source`) plus what the **broker** stamped on the delivery (`sequenceNumber`, `deliveryCount`). The broker-stamped pair is admissible on the same rule that excludes `messageId`: it is assigned by the broker, not chosen by the producer, and it identifies a delivery rather than a person — `sequenceNumber` is the only handle joining this service's log index to the queue, and the only one at all on lines written during a store outage, where the body is deliberately never read. The receipt line additionally carries broker timings (`enqueuedTime`, `lockedUntil`), the message's own `sharedTime`, and `attributedTo` — a bounded two-value label naming *which* identity a run was made as and never the user id, which stays in a `CJSCPPUID` header (`CallerIdentityTest`, `TelemetryPrivacyTest`). No payload logging; metric labels are low-cardinality enumerations and carry no identifiers. The walking skeleton handles no defendant data at all (thin message only), which the privacy tests still pin. |
| VIII | Estate Conventions | **PASS** — Gradle wrapper, package `uk.gov.hmcts.cp.informantregister`, Conventional Commits, Jira-prefixed branch, SLF4J + Logback JSON. PMD and the coverage report are no longer conventions on trust: `./gradlew pmdMain` and `./gradlew jacocoTestReport` are explicit steps in `.github/workflows/ci-build-publish.yml`, so a PMD failure blocks CI. *(Amended during implementation, 21 Aug 2026: Checkstyle google_checks and the JaCoCo coverage verification were adopted into `check`/`build` by explicit user decision as part of the CI-policy alignment — constitution 1.0.2.)* |
